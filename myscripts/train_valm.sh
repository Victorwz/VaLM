...
CKPT_DIR=./checkpoint_valm
DATASTORE_DIR=./image_feature_datastore_200M
K=$1
...
python -m torch.distributed.launch --nproc_per_node=16 \
    ...
    --use-knn-datastore --load-knn-datastore --dstore-fp16 --dstore-size 191504486 --k ${K} --probe 32 --dstore-filename ${DATASTORE_DIR} \
    --use-joint-attention --joint-layer-index 2 \
    --ddp-backend legacy_ddp --use-gpu-to-search 